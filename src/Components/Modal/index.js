...
const Modal = ({ jamId, jamName, adminName, modalType, invitationInfo, modalState, message }) => {

    const { firstName, lastName } = invitationInfo;

    let history = useHistory();

    const onSubmit = () => {        
        DataService.newTenantInvitation(jamId, invitationInfo)
        .then((res) => {
            const invId = res.id;
    ...
    return (
        <div className="modal-wrapper">
           ...
           <div className="modal-buttonsArea">
               <ButtonPlain text="Cancel" fn={Cancel} />
               <ButtonPlain text="Send anyway" fn={onSubmit}/>
           </div>
        </div>
    );
};
...
const mapStateToProps = state => {
    ...
    const { adminName, jamName,  } = state.jamInfo
    const { modalType } = state.modal;

    return { jamId, userId, jamName, modalType, userName, userRole, adminName  };
};
